...
int main(int argc, char **argv) {
  ...
  cfg.kf_min_dist = cfg.kf_max_dist = 8;
...
// Initialize codec.
  ...
  {
    const int max_intra_size_pct = 100;
      ...
      switch (pkt->kind) {
        case VPX_CODEC_CX_FRAME_PKT:
          for (i = cfg.ts_layer_id[frame_cnt % cfg.ts_periodicity];
            ...
            printf("stream: %d key: %d frame: %d size: %d\n",
                    i, pkt->data.frame.flags & VPX_FRAME_IS_KEY, frame_cnt, pkt->data.frame.sz);
